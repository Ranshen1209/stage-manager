...
- **远程切屏与可读尺寸**：结合 Windows 缩放比例和可用空间渐进缩小大窗口，给低分辨率屏幕留出边缘空间，同时保护小窗口的可读尺寸。位置保留靠边或居中关系，往返始终从原布局基准计算，保留分组。
...
- **手动游戏模式**：设置页或托盘一键切换，暂停自动调度、实时预览和输入监听，隐藏侧栏；保留分组及原有功能设置，默认关闭。
- **记住布局**：重启本应用后恢复仍然打开的窗口分组、位置与最近成员；退出时恢复接管前的布局。

## 开始使用

当前版本为 **[0.3.18](https://github.com/Ranshen1209/stage-manager/releases/tag/v0.3.18)**，更新内容见 [版本记录](docs/changelog.zh-CN.md)。

0.3.18 减少暂停、退出及重新开启时的重复窗口恢复，普通和最小化窗口采用不激活的恢复方式，减少对任务栏图标顺序的干扰。当前不提供 Explorer 任务栏排序快照或回滚。
...
游戏前可在 **“设置 → 游戏模式”** 或右键托盘菜单中开启游戏模式。该模式不改变当前应用窗口的显示状态，会暂停本程序的自动调度、窗口扫描、实时预览、全局快捷键、中键手势和侧栏唤出。手动关闭后恢复原设置；选择跨重启保存，已暂停的调度不会因此自动开启。需要管理界面时可从托盘打开总览。
...
0.3.18 的普通构建与混淆加壳后的 EXE 均通过 294 项逻辑测试。新增 14 项覆盖恢复时跳过无变化窗口、不激活的恢复方式、原始最小化／最大化状态、反复暂停与重新开启，以及切组未完成时的异步命令。按要求未运行窗口、Explorer 任务栏、真实 DPI 切换或远程连接测试；实际效果仍受应用自身及 Windows 行为影响。
