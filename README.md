...
- **远程切屏与可读尺寸**：屏幕稳定后，按 Windows 缩放比例保留窗口的逻辑尺寸，放不下时才缩小；位置按剩余空间调整，保留靠边或居中关系。大屏留出更多空间，小屏提高窗口占比；支持分辨率 / 缩放动态调整、手动修改新基准及反复往返，保留分组。
...
当前版本为 **[0.3.14](https://github.com/Ranshen1209/stage-manager/releases/tag/v0.3.14)**，更新内容见 [版本记录](docs/changelog.zh-CN.md)。
...
0.3.14 通过 246 项逻辑测试、Release 构建和 XAML 静态检查，覆盖逻辑尺寸保留、混合 DPI、连续往返、重启恢复，以及游戏模式的调度隔离、持久化、快捷键释放和恢复冲突。本版未运行窗口、真实游戏、DPI 切换或远程连接测试；实际效果受应用最小尺寸及显示驱动行为影响。
